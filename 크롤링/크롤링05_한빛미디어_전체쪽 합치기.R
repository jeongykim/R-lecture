...
# query <- sprintf('?page=%d&cate_cd=&srt=&searchKey=&keyWord=', 1) #sprintf로 %d 자리에 정수 1을 넣어준다.
...
for (page in c(1:25)) {
  ...
  for (li in lis) {
    ...
    print(paste(title, price, sep = ',')) #에러를 찾기 위해서 했음
...
write.csv(new_books, '한빛도서.csv',row.names=F) #인코딩이 깨지면 fileEncoding='utf-8' 옵션을 걸어준다. 
df <- read.csv('한빛도서.csv')
